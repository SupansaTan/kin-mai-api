...
namespace KinMai.UnitTests.Controllers.AuthenticationControllerTest
{
    public class SendEmailResetPassword
    {
        ...
        [Fact]
        public async Task SendEmailResetPassword_ReturnStatus200_WhenEmailIsExist()
        {
            // arrange
            ...
            var mockRequest = new ForgotPasswordRequest()
            {
                Email = "nampunch1@gmail.com"
            };
            ...
            // act
            var actualOutput = await authenticationController.SendEmailResetPassword(mockRequest);
        ...
        [Fact]
        public async Task SendEmailResetPassword_ReturnStatus400_WhenEmailDoesNotExist()
        {
            // setup database response
            mockEntityUnitOfWork.Setup(x => x.UserRepository.GetSingleAsync(It.IsAny<Expression<Func<User, bool>>>()))
                                .ReturnsAsync(() => null);
            ...
            {
                Data = false,
                Message = "This email have not registered KinMai account before.",
                Status = 400
            };
